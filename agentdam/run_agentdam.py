...
def config() -> argparse.Namespace:
    ...
    # privacy config
    parser.add_argument("--privacy_test", action="store_true", help="enable privacy test")
    parser.add_argument(
        "--privacy_config_path",
        type=str,
...
async def test(
    ...
    privacy_scores = []
    privacy_eval = PrivacyEvaluator(Path(args.privacy_config_path)) if args.privacy_test else None
    ...
    for config_file in config_file_list:
        try:
            ...
            with open(config_file) as f:
                ...
                if args.privacy_test:
                    try:
                        ...
                        raise KeyError("The field sensitive_data must be in config json if privacy_test enabled")
                else:
                    print("[Warning!] Privacy Tests are disabled! Please rerun with --privacy_test.")
            ...
            while True:
                ...
                if args.privacy_test:
                    privacy_eval_score, privacy_eval_txt = privacy_eval.test(action_str=action_str, sensitive_data=sensitive_data)
            ...
            logger.info(f"Performance score so far: {sum(scores)} / Out of: {len(scores)}")
    ...
    if len(privacy_scores):
        logger.info(f"Total privacy leakages: {sum(privacy_scores)} / Out of: {len(privacy_scores)}")
    print(f"Elapsed time (s): {datetime.now() - start_time}")
